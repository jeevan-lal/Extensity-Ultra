...
export const extensionDetails = {
  ...
  prod: {
    ...
    // Server Site
    optionURL: "https://example.netlify.app/"
  },
...
export function manifestTransform(json: ManifestJSON): object {
  ...
  if (process.env.BROWSER_TYPE !== "firefox") {
    if (exconfig.id && exconfig.matchesURL && exconfig.matchesURL.length > 0) {
      ...
      if (!json["externally_connectable"]["ids"]) json["externally_connectable"]["ids"] = [];
      if (!json["externally_connectable"]["matches"]) json["externally_connectable"]["matches"] = [];
