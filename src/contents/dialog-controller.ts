  ...
  try {
    ...
    const efAD = {
      ...
      checkMessageType(msg: string) {
         ...
         */
        const successMessageList = [/record successfully/, /record save successfully/, /saved/];
        const isMatchS = successMessageList.some(rx => rx.test(msg.trim().toLocaleLowerCase()));
        if (isMatchS) {
          return "s";
        }
        ...
        const isMatchD = errorMessageList.some(rx => rx.test(msg.trim().toLocaleLowerCase()));
        if (isMatchD) {
          return "d";
        }
    ...
    const on = localStorage.getItem(_store.keys.status);
    if (!on) return false;
