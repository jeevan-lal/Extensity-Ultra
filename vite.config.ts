    ...
    build: {
      ...
      rollupOptions: {
        input: {
          ...
          // Background
          "service-worker": resolve(__dirname, 'src/background/background.ts'),
        },
        output: {
          ...
          sourcemap: false,
          // https://rollupjs.org/configuration-options/#output-sanitizefilename
          sanitizeFileName: (fileName) => {
            fileName = fileName.replace(/^[\x00\?:*]+/, '');
            return fileName.replace(/[\x00\?:*]/g, '-');
          },
          // Allows you to create shared chunks for frequently used dependencies.
